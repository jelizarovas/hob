...
// firebase deploy --only functions:updateUserPhoto
export const updateUserPhoto = onRequest((req, res) => {
  corsHandler(req, res, async () => {
    if (req.method !== "POST") {
      res.status(405).json({ error: "Method Not Allowed" });
      return;
    }
    // Only admins or managers can update other users' photoURL
    if (!(await verifyAdminOrManager(req, res))) return;
    ...
    if (!uid || !photoURL) {
      res.status(400).json({ error: "Missing uid or photoURL" });
      return;
    }
    try {
      await admin.auth().updateUser(uid, { photoURL });
      res.status(200).json({ message: `User ${uid} photo updated` });
    } catch (error) {
